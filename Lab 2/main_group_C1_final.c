    ...
    */
    ...
    int process_msg(char** pMsg, int* msgLen, char** pDestInc, char direction, bool* pModCon)
    {
        ...
        */
        ...
        regex_t regBW, regHost, regCon, regConTest, regCont, regType, regWww, regDoubleHttp;
        int retv;
        ...
        if((retv = regcomp(&regBW, "[N|n]orrk.*ping|[S|s]ponge[B|b]ob|(Britney Spears)|(Paris Hilton)", REG_ICASE|REG_EXTENDED)))
        {
            printf("Reg exp comp. fail\n");
        }
        ...
        if((retv = regcomp(&regWww, "www.*|(Host.*[\\.].*[\\.].*)", REG_ICASE|REG_EXTENDED)))
        {
            printf("Reg exp comp. fail\n");
        }
		if((retv = regcomp(&regDoubleHttp, "([^( )](http)[^(( )|(\\\r\\\n)]*)", REG_ICASE|REG_EXTENDED)))
        {
            printf("Reg exp comp. fail\n");
        }
        ...
        do
        {
            ...
            //Check for "double http://" errors
            regmatch_t foundHttpAt[4];
            if(!regexec(&regDoubleHttp, pLine, 4, foundHttpAt, 0))
            {

              if(foundHttpAt[0].rm_so != ((unsigned int)-1))
              {
                int startAt = foundHttpAt[0].rm_so;
                int endAt = foundHttpAt[0].rm_eo;

                  //Filter out second http to end (saving \r\n)
                  memmove(&pLine[startAt+1],&pLine[endAt],lineLen-endAt+2);
                  lineLen -= endAt-startAt-1;
            ...
            if((direction == OUTGOING) && !(regexec(&regBW, pLine, 0, NULL, 0)))
            {
                ...
                printf("BAD KEYWORD FOUND IN URL! EXECUTING COUNTERMEASURES TO PREVENT BRAIN DECAY!\n");
                ...
                memcpy(pNewMsg, REDIRECTHEADER_BADURL, REDIRECTLEN);
            ...
            {
                bool wwwFound = false;
                // Check for "www" and other subdomains.
                // This part "might" be redundant as we handle the "double http" error above. Disabled now, enabled if DEBUG is defined

#ifdef DEBUG
                if(!(regexec(&regWww, pLine, 0, NULL, 0)))
                {
                    wwwFound = true;
                }
                ...
                if(wwwFound)
                  ...
                  pDest = (char *) malloc(lineLen - 7 + 4);

                // Extract host URL/destination (do not copy \r\n)
                // If no www. subdomain is given (or other no subdomain was found)
                // we "correct" the GET message by adding the www.
                if(!wwwFound)
                {
                  memcpy(pDest,"www.",4);
                  memcpy(&pDest[4],&pLine[6],lineLen-8);
                  // Null terminate the "dest" string
                  pDest[lineLen-8+4] = '\0';

                  char* tempLine = (char *) malloc(lineLen+4);
                  memcpy(tempLine, pLine, 6);
                  memcpy(&tempLine[6],"www.",4);
                  memcpy(&tempLine[10],&pLine[6],lineLen-6);

                  // Write to msg
                  memcpy(&pNewMsg[newMsgLen],tempLine,lineLen+4);
                  newMsgLen += lineLen+4;
            ...
            if((direction == INCOMMING) && (lineLen == 2) && (strcmp(pLine,"\r\n") == 0))
            {
                ...
                if(textType)
                {
                    ...
                    {
                        printf("BAD KEYWORD FOUND IN CONTENT! EXECUTING COUNTERMEASURES TO PREVENT BRAIN DECAY!\n");
                        ...
                        // Allocate space for the redirect message
                        pNewMsg = (char *) malloc(REDIRECTLEN);
                        memcpy(pNewMsg, REDIRECTHEADER_BADWORD, REDIRECTLEN);
                        // Set msgLen of the message
                        *msgLen = REDIRECTLEN;
                        // Change pMsg pointer
                        *pMsg = pNewMsg;
